 ...
 *
 * $Id: ArrayListSerializer.java,v 1.1.1.1 2004/03/31 14:21:01 mclark Exp $
 *
...
class ArrayListSerializer extends Serializer
{
	...
	String type = jso.getString("java_class");
	if(!type.equals("java.util.ArrayList"))
	    throw new UnmarshallException("not an ArrayList");
	...
	try {
	    for(; i < jsa.length(); i++)
		al.add(unmarshall(null, jsa.get(i)));
	} catch (UnmarshallException e) {
	    throw new UnmarshallException
		("element " + i + " " + e.getMessage());
	}
	...
	obj.put("java_class", al.getClass().getName());
	obj.put("arr", arr);
	int i=0;
	try {
	    for(; i < al.size(); i++) arr.put(marshall(al.get(i)));
