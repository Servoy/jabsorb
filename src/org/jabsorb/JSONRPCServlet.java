 ...
 * </pre>
 * 
 * </p> The JSONRPCServlet looks for a session specific bridge object under the
 * attribute <code>"JSONRPCBridge"</code> in the HttpSession associated with the
 * request (without creating a session if one does not already exist). If it
 * can't find a session specific bridge instance, it will default to invoking
 * against the global bridge. </p>
 * <p>
...
public class JSONRPCServlet extends HttpServlet
{
   ...
   * attempt to gzip the response if it can. Gzipping smaller responses is
   * counter productive for 2 reasons: 1. if the response is really small, the
   * gzipped output can actually be larger than the non-compressed original.
   * because of the gzip header and the general overhead of the gzipping. This
   * is a lose-lose situation, so the original should always be sent in this
   * case. 2. gzipping imposes a small performance penality in that it takes a
   * little more time to gzip the content. There is also a corresponding
   * penality on the browser side when the content has to be uncompressed. This
   * penalty is really small, and is normally more than outweighed by the
   * bandwidth savings provided by gzip (the response is typically 1/10th the
   * size when gzipped! Especially for json data which tends to have a lot of
   * repetition. So, the GZIP_THRESHOLD should be tuned to a size that is
   * optimal for your application. If your application is always served from a
   * high speed network, you might want to set this to a very high number-- (or
   * even -1 to turn it off) for slower networks where it's more important to
   * conserve bandwidth, set this to a lower number (but not too low!) Set this
   * to zero if you want to always attempt to gzip the output when the browser
   * can accept gzip encoded responses. This is useful for analyzing what a good
   * gzip setting should be for potential responses from your application. You
   * can set this to -1 if you want to turn off gzip encoding for some reason.
   */
  ...
  /**
   * Creates a new JSONRPCServlet with the bridge in the "JSONRPCBridge"
   * variable
   */
   ...
   * 
   * @param bridgeLocation The location of the JSONRPCBridge variable in the
   *          session
   */
  ...
  /**
   * Called by the container when the servlet is initialized. Check for optional
   * configuration parameters.
   * <p>
   * At this time, only gzip_threshold is looked for.
   * </p>
   * <p>
   * If it is found, and a valid Integer is specified, then that is used for the
   * GZIP_THRESHOLD.
   * </p>
   * <p>
   ...
   * </p>
   * <p>
   * Set this to -1 if you want to disable gzip compression for some reason, or
   * if you have another filter or other mechanism to handle gzipping for you.
   * </p>
   * <p>
   * Set this to 0 to attempt to gzip all responses from this servlet.
   * otherwise, set it to the minimum response size at which gzip compression is
   * attempted.
   * </p>
   * <p>
   * <b>NOTE:</b> if the browser making the request does not accept gzip
   * compressed content, or the result of gzipping would cause the response size
   * to be larger (this could happen with very small responses) then the content
   * will be returned without gzipping, regardless.
   * </p>
   * <p>
   * of this setting, so it is very reasonable idea to set this to 0 for maximum
   * bandwidth savings, at the (very minor) expense of having the server attempt
   * to gzip all responses.
   * </p>
   * 
          ...
          if (bout.length <= gzippedOut.length)
          {
            log
                .warn("gzipping resulted in a larger output size!  "
                    ...
                    + " original size = " + bout.length + "  gzipped size = "
                    + gzippedOut.length);
          }
          else
          {
  ...
  private byte[] gzip(byte[] in)
  {
    if (in != null && in.length > 0)
    {
      ...
      try
      {
        ...
        if (log.isDebugEnabled())
        {
          log.debug("gzipping took " + (System.currentTimeMillis() - tstart)
              + " msec");
        }
   ...
   * @return a String containing the formatted json text for the passed in json
   *         object.
   */
