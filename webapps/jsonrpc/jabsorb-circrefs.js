...
var jabsorb_circrefs = function()
{
  ...
  /**
   * if this is true, circular references in the object graph are fixed up if
   * this is false, circular references cause an exception to be thrown
   */
  pub.fixupCircRefs = true;

  /**
   * if this is true, duplicate objects in the object graph are optimized if
   * it's false, then duplicate objects are "re-serialized"
   */
   ...
   * @param resultKey
   *          With what key the result should be stored
  ...
  {
    // to detect circular references and duplicate objects, each object has a
    // special marker
    // added to it as we go along.

    // therefore we know if the object is either a duplicate or circular ref if
    // the object
    // already has this marker in it before we process it.

    // the marker object itself contains two pointers-- one to the last object
    // processed
    // and another to the parent object
    // therefore we can rapidly detect if an object is a circular reference
    // by following the chain of parent pointer objects to see if we find the
    // same object again.

    // if we don't find the same object again in the parent recursively, then we
    // know that it's a
    // duplicate instead of a circular reference

    // the pointer to the last object processed is used to link all processed
    // objects together
    // so that the marker objects can be removed when the operation is complete

    // once all objects are processed, we can traverse the linked list, removing
    // all the markers
    ...
    // fixups detected as we go along, both for circular references and
    // duplicates
    var fixups = [];

    // unlink the whole chain of marker objects that were added to objects when
    // processing
   ...
   * @param data
   *          The data which contains fixups
   ...
   * @param fixups
   *          array of fixups to apply. Each element of the array is a 2 element
   *          array, containing the array with the fixup location followed by an
   *          array with the original location to fix up into the fixup
   *          location.
   */
